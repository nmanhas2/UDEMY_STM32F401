...
///	Purpose: Utilizing USART2 to transmit data via USB
...
static uint16_t compute_uart_bd(uint32_t PeriphClk, uint32_t BaudRate)
{
	//i'm not sure where he got this method (from the UDEMY course) of calculating the baudrate
	//the reference manual explains it in Section 19.3.4, which is what i used in the custom library i made
