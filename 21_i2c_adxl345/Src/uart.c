...
#define DMA_SEN				(1U<<0) //enable bit for StreamX for DMA
#define CHSEL4				(1U<<27) //bit to enable channel 4 for DMA StreamX
#define MINC_MODE			(1U<<10) //bit for enabling memory increment mode for DMA StreamX
#define DIR_MEM_PERIPH		(1U<<6) //bit for setting DMA StreamX direction to memory to peripheral
#define CR_TCIE				(1U<<4) //bit to enable transfer complete interrupt for DMA StreamX

#define UART_DMAT			(1U<<7) //bit to enable DMA enable transmitter
...
//initializing DMA1 stream6 for uart2
void dma1_stream6_init(uint32_t src, uint32_t dst, uint32_t len)
{
	//enable clock access to DMA1, located on AHB1 bus based on section 6.3.9 in refernece manual
	RCC->AHB1ENR |= (1U<<21);

	//disable DMA1 Stream6, based on section 9.5.5 in reference manual
	DMA1_Stream6->CR &= ~DMA_SEN;

	//wait until DMA1 Stream6 is disabled
	while(DMA1_Stream6->CR & DMA_SEN);

	//clear all interrupt flags for Stream6, based on section 9.5.4 in reference manual
	DMA1->HIFCR &= ~((1U<<16) | (1U<<18) | (1U<<19) | (1U<<20) | (1U<<21));

	//set the destination buffer, based on section 9.5.7 in reference manual
	DMA1_Stream6->PAR = dst;

	//set the source buffer, based on section 9.5.8 in reference manual
	DMA1_Stream6->M0AR = src;

	//set the length, based on section 9.5.6 in reference manual
	DMA1_Stream6->NDTR = len;

	//select Stream6 CH4, based on section 9.5.5 in reference manual
	//clearing the rest of the bits in the register as well
	DMA1_Stream6->CR = CHSEL4;

	//enable memory increment, based on section 9.5.5 in reference manual
	DMA1_Stream6->CR |= MINC_MODE;

	//set transfer direction (memory to peripheral), based on section 9.5.5 in reference manual
	DMA1_Stream6->CR |= DIR_MEM_PERIPH;

	//enable transfer complete interrupt, based on section 9.5.5 in reference manual
	DMA1_Stream6->CR |= CR_TCIE;

	//enable direct mode and disable FIFO, based on section 9.5.10 in reference manual
	DMA1_Stream6->FCR = 0;

	//enable DMA1 Stream6, based on section 9.5.5 in reference manual
	DMA1_Stream6->CR |= DMA_SEN;

	//enable UART2 DMA transmitter, based on section 19.6.6 in reference manual
	USART2->CR3 |= UART_DMAT;

	//DMA interrupt enable in NVIC
	NVIC_EnableIRQ(DMA1_Stream6_IRQn);
}

//Function to initialize USART2 tx and rx
